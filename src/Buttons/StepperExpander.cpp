...
// the next step is due and determine the sequence of phases that we need.
    ...
    // Linear Position
    void StepperExpander::moveToPos(unsigned int pos){
    powerUp();
    ...
    void StepperExpander::moveToRev(unsigned int pos){
    powerUp();
    stepstogo = pos - currentrev;
    ...
    void StepperExpander::stepCW()
    {
        ...
        currentstep++;
        currentrev++;
        if (currentrev == stepsperrev) currentrev = 0;
    ...
    if (currentrev < 0) currentrev = stepsperrev - 1;
    }
    ...
    int StepperExpander::currentRevolution()
    {
    return currentrev;
    }
    // Returns steps to go
